  ...
  const checkSupabaseAuth = async () => {
    try {
      ...
      if (sessionError) {
        ...
        authStatus = `認証エラー: ${sessionError.message}`;
      } else if (sessionData?.session) {
        const expiryTime = new Date((sessionData.session?.expires_at || 0) * 1000);
        const now = new Date();
        const remainingMins = Math.round((expiryTime.getTime() - now.getTime()) / (1000 * 60));
        
        authStatus = `認証済み (トークン残り約${remainingMins}分)`;
        
        // トークンの残り時間が10分未満なら更新を試みる
        if (remainingMins < 10) {
  ...
  const fetchNotes = async () => {
    ...
    try {
      ...
      try {
        ...
        if (countError) {
          console.error("総数カウントエラー:", countError);
          // エラーがあっても続行
        } else {
        ...
        if (allNotesError) {
          console.error("全ノート取得エラー:", allNotesError);
          // エラーがあっても続行
        } else {
        ...
        console.error("クエリエラー:", queryErr);
        setError(`データ取得エラー: ${queryErr.message || '不明なエラー'}`);
        setDebugInfo({ error: queryErr, message: queryErr.message, session });
      }
    } catch (err: any) {
      console.error('ノート取得中にエラーが発生しました:', err);
      setError(`ノートの読み込みに失敗しました: ${err.message || '不明なエラー'}`);
      setDebugInfo({ error: err, message: err.message, session });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCreateNote = async (e: FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        alert(`ノートの作成に失敗しました: ${error.message}`);
        throw error;
      }
      ...
      alert(`ノートの作成に失敗しました: ${err.message || '不明なエラー'}`);
    } finally {
      setIsCreating(false);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-6 p-4 bg-gray-100 rounded text-xs">
        ...
        <button 
          onClick={fetchNotes}
          className="mt-2 bg-gray-500 text-white px-2 py-1 rounded-md text-xs"
        >
          データ再取得
        </button>
        {debugInfo && (
